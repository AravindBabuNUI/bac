...
function renderInputWithErrorControl() {
  function Wrapper() {
    const { control, setError, clearErrors } = useForm<FormValues>({ defaultValues: { field: '' } });
    return (
      <div>
        <Input control={control} name="field" label="Field" />
        <button onClick={() => setError('field', { type: 'manual', message: 'Required' })}>set-error</button>
        <button onClick={() => clearErrors('field')}>clear-error</button>
      </div>
    );
  }
  return render(<Wrapper />);
...
describe('Input', () => {
  ...
  it('error: border-error class is removed once the error clears', async () => {
    renderInputWithErrorControl();
    await act(async () => fireEvent.click(screen.getByText('set-error')));
    expect(screen.getByRole('textbox')).toHaveClass('border-error');
    await act(async () => fireEvent.click(screen.getByText('clear-error')));
